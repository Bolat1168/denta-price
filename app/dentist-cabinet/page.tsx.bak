...
import type { AlmatyServicePrice, PromotionPriceMap } from '../../pricing/types';
import { getSegmentConfigWithRealRanges } from './logic';
...
export default function DentistCabinetDesktopCompositionPage() {
  ...
  // Mobile analytics height state and refs for resizer
  const [mobileAnalyticsHeight, setMobileAnalyticsHeight] = useState(300);
  const isMobileDraggingRef = useRef(false);
  const mobileStartYRef = useRef(0);
  const mobileStartHeightRef = useRef(0);

  const [priceLoadingMap, setPriceLoadingMap] = useState<Map<string, boolean>>(new Map());

  const setPriceLoading = (rowId: string, isLoading: boolean) => {
    setPriceLoadingMap(prev => new Map(prev).set(rowId, isLoading));
  };
  ...
  useEffect(() => {
    ...
    setPriceLoading(analyticsRowId, true);
  ...
  useEffect(() => {
    ...
    const fetchNicheStatuses = async () => {
      const activeRows = serviceRows.filter(r => r.serviceId && r.price !== null && r.segment && r.radiusKm && !nicheStatuses[r.id]);
      for (const row of activeRows) {
        if (abortController.signal.aborted) break;
        setPriceLoading(row.id, true);
        try {
    ...
    const fetchData = async () => {
      ...
      setServiceRows(prev => {
        ...
        while (newRows.length < 3) {
          newRows.push(createEmptyServiceRow());
        }
        return newRows;
      });
      isInitialLoad.current = false;
  ...
  // Mobile resizer handlers
  const handleMobileResizerMouseDown = useCallback((e: React.MouseEvent) => {
    isMobileDraggingRef.current = true;
    mobileStartYRef.current = e.clientY;
    mobileStartHeightRef.current = mobileAnalyticsHeight;
    e.preventDefault();
  }, [mobileAnalyticsHeight]);

  const handleMobileResizerMouseMove = useCallback((e: MouseEvent) => {
    if (!isMobileDraggingRef.current) return;
    const deltaY = mobileStartYRef.current - e.clientY;
    const newHeight = Math.max(200, Math.min(600, mobileStartHeightRef.current + deltaY));
    setMobileAnalyticsHeight(newHeight);
  }, []);

  const handleMobileResizerMouseUp = useCallback(() => {
  ...
  useEffect(() => {
    const handleGlobalMouseMove = (e: MouseEvent) => {
      if (isMobileDraggingRef.current) {
        handleMobileResizerMouseMove(e);
      }
    };

    const handleGlobalMouseUp = () => {
      if (isMobileDraggingRef.current) {
        handleMobileResizerMouseUp();
      }
    };
  ...
  }, [handleMobileResizerMouseMove, handleMobileResizerMouseUp]);
  ...
  const handleServiceSelect = (rowId: string) => {
    ...
    const selectedRow = serviceRows.find(r => r.id === rowId);
    if (selectedRow && selectedRow.serviceId && selectedRow.price !== null && selectedRow.segment && selectedRow.radiusKm) {
      setPriceLoading(rowId, true);
      fetch(`/api/niche-status?serviceId=${selectedRow.serviceId}&segment=${selectedRow.segment}&radius=${selectedRow.radiusKm}&price=${selectedRow.price}`)
        .then(async res => {
          if (res.ok) {
            const data = await res.json();
      ...
      setTimeout(() => {
        if (newRow.serviceId && newRow.price !== null && newRow.segment && newRow.radiusKm) {
          ...
          fetch(`/api/niche-status?serviceId=${newRow.serviceId}&segment=${newRow.segment}&radius=${newRow.radiusKm}&price=${newRow.price}`)
            ...
            .catch(console.error)
            .finally(() => setPriceLoading(newRow.id, false));
        }
      }, 0);
      return updated;
    });
...
const handleSaveProfile = async () => {
    ...
    try {
        ...
        console.log('Saving profile for:', dentistId, payload);
  ...
  useEffect(() => {
    const loadStats = async () => {
      ...
      try {
        const res = await fetch(`/api/stats/get-dentist-stats?dentistId=${dentistId}`);
        if (res.ok) {
          const stats = await res.json();
          setServiceRows(prev => prev.map(row => ({
            ...row,
            views1km: stats.byService[row.serviceId]?.views1km || 0,
            views3km: stats.byService[row.serviceId]?.views3km || 0,
            views6km: stats.byService[row.serviceId]?.views6km || 0,
            viewsCity: stats.byService[row.serviceId]?.viewsCity || 0,
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', width: '100%', height: '100%' }}>
                                    <input
                                      ...
                                      onKeyDown={(e) => {
                                        if (e.key === 'Enter') {
                                          e.preventDefault();
                                          const cleanVal = e.currentTarget.value.replace(/\D/g, '');
                                          updateServicePrice(row.id, cleanVal);
                                        }
                                      }}
                                      onClick={(e) => e.stopPropagation()}
                          ...
                          if (selectedRow && selectedRow.serviceId && selectedRow.price !== null && selectedRow.segment && selectedRow.radiusKm) {
                            ...
                            fetch(`/api/niche-status?serviceId=${selectedRow.serviceId}&segment=${selectedRow.segment}&radius=${selectedRow.radiusKm}&price=${selectedRow.price}`)
                              .then(async res => {
                                if (res.ok) {
                                  const data = await res.json();
                                  setNicheStatuses(prev => ({ ...prev, [selectedId]: data }));
                                }
                              })
                              .catch(console.error)
                              .finally(() => setPriceLoading(selectedId, false));
                          }
                        }}
                      >
                        {serviceRows.map((row) => (
